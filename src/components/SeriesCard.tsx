...
export function SeriesCard({ series, onDelete, onAddStory }: SeriesCardProps) {
  ...
  const getSeriesColor = (seriesId: string) => {
    ...
    return {
      ...
      border: `hsla(${hue}, 85%, 75%, 1)`,
      accent: `hsla(${hue}, 85%, 85%, 1)`,
  ...
  return (
    <div 
      className="rounded-lg bg-muted hover:bg-accent/50 transition-colors group overflow-hidden"
      style={{ 
        borderLeft: `4px solid ${seriesColors.border}`,
      }}
    >
      <div className="p-6">
        <Collapsible
                    ...
                    <div className="space-y-3">
                      {seriesWithStories.stories.map((story, index) => (
                        <motion.div 
                          ...
                          style={{
                            borderLeft: `3px solid ${seriesColors.border}`
                          }}
                        >
                          <div className="flex items-start justify-between">
                            ...
                            <ArrowRight className="h-5 w-5 text-muted-foreground mt-1 ml-2 flex-shrink-0" />
                          </div>
                        </motion.div>
                      ))}
                    </div>
